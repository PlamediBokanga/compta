...
export function InvoicesPage() {
  ...
  return (
    <div className="space-y-5">
      ...
      {loading && Array.from({ length: 5 }).map((_, index) => <tr key={index}><td colSpan={6} className="px-4 py-3"><div className="skeleton h-10" /></td></tr>)}
      {!loading && filtered.length === 0 && <tr><td colSpan={6} className="px-4 py-10 text-center text-ink-500">Aucun {isInvoicesTab ? 'facture' : 'devis'} pour le moment.</td></tr>}
